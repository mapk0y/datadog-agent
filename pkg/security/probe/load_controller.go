...
func (lc *LoadController) CountFork(event *Event) {
	...
	cookie := uint32(event.Process.ResolveCookie(event))
	if cookie == 0 {
		// nothing to do
		return
	}

	entry, ok := lc.forkCounters.Get(forkLRUKey{Cookie: cookie})
	if ok {
		counter := entry.(*uint64)
		if atomic.AddUint64(counter, 1) >= uint64(lc.ForkBombThreshold) {
			forkBomb = true
		...
		newCount = uint64(1)
		lc.forkCounters.Add(forkLRUKey{Cookie: cookie}, &newCount)
	}

	if forkBomb {
		lc.probe.DispatchCustomEvent(NewForkBombEvent(event, time.Now()))

		// drop fork events with the given cookie in kernel space
		lc.probe.resolvers.ProcessResolver.MarkCookieAsBestEffort(cookie)
	}
}
